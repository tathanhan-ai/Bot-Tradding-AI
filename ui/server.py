    ...
    def evaluate_candidate_pipeline(self, candidate: CandidateOrder, research: Optional[AIOrderResearchResult] = None, snapshot=None, trace=None):
        ...
        def tactical_lessons_gate(order: CandidateOrder, _snapshot: MarketSnapshot) -> StageOutcome:
            ...
            try:
                ...
                smc_bias = getattr(self.ai_verdict, "smc_bias", "") or ""
                sweep_info = getattr(self.ai_verdict, "last_sweep_info", "") or ""
                has_sweep = bool(sweep_info) and ("Quét" in sweep_info or "quet" in sweep_info.lower() or "$" in sweep_info)
